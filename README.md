...
《我的修仙日记》是一款使用 Cocos Creator 3.8.8 开发的竖屏修仙放置小游戏。当前 `0.10.0` 开发基线处于第一阶段核心循环收尾期，已经接通本地客户端、Fastify 服务端、PostgreSQL 持久化和 Redis 健康检查，发布目标为微信小游戏。
...
- 仅开发构建可见的诊断面板：显示同步、生命周期、队列和修炼快照，并可通过真实服务端结算模拟离线 1/8/24 小时

伴侣、洞府、真实排行榜、深度装备养成、副本、商业化和正式部署尚未进入完整实现。

当前第一阶段收尾顺序为：完成微信开发者工具和真机验收；经验/资源注入、网络故障模拟、固定随机种子等其余开发调试控制随后开发。
...
当前自动化基线为共享领域 29 项、服务端及客户端单元 106 项、PostgreSQL 集成 25 项，共 160 项。
...
当前源码对应的 `0.10.0-r1` 微信小游戏 debug/release 产物已完成构建和 JavaScript 语法检查；debug 包包含离线 1/8/24 小时入口，release 包不包含 `DebugRoot`、调试面板标题或按钮文案；尚未完成微信开发者工具和真机验收。
