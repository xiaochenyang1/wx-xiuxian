...
Lv.11 那一行几乎不动（18,205 → 17,990），意味着早期数值不需要重新调平衡；而配装占比从"衰减到 0"变成恒定 87.77%，这条线以后不会再衰减。起步装（六槽普通 +0、四功法普通 1 星）逐件累加为 2,809 bp，即 +28.09%，与改造前 Lv.11 的 +625 固定值（+28.41%）对齐；两种模型下 Lv.11 起步装总战力分别是 2,825 与 2,817。

起步装的功法 base 之和是**普通**四本的 40+35+55+45 = 175，不是满配那一行的 425 —— 后者是优秀四本的值。初稿在起步装这一行误用了 425，因而把 2,809 写成 3,935、把 625 写成 875。校准结论不受影响：两种模型在 Lv.11 起步装下相差 8 点（0.3%），仍然是"早期数值不需要重新调平衡"。

所有 bp 数值均按逐件 `floor` 后求和，不能先求 base 之和再乘倍率——两种算法会因舍入产生数点差异，测试断言以逐件累加为准。

本节与第 4 节的全部数字已按当前仓库配置（`ASSET_QUALITY_MULTIPLIER_BP`、`TECHNIQUE_STAR_MULTIPLIER_BP`、`EQUIPMENT_CONFIGS`、`TECHNIQUE_CONFIGS`、境界 `powerMultiplier`）用 BigInt 复刻 `scaleByBasisPoints` 独立验算过，逐项吻合：满配 71,774 bp、起步装 2,809 bp、改造前满配 16,005、改造前起步装 625、Lv.11/100/1000 三点占比均为 87.77%、Lv.11 满配 17,990、Lv.11 起步装改造前 2,825 改造后 2,817、Lv.15 起步装 3,842、Lv.100 满配 817,740、半数配装 458,870；塔的门槛、34 层累计灵石 1,538,704 与两列最早可通等级同样逐项吻合。**实施时这些期望值是标尺，测试不符应先怀疑实现，而不是改断言。**
...
- 单件派生字段 `techniques[].fixedPower` 与 `equipment[].fixedPower` 同样改名 `powerBonusBp`，类型由 `BigNumberString` 改为 `number`。它们是 `calculateTechniqueContribution` / `calculateEquipmentContribution` 的直接输出，`LoadoutBonuses` 改字段后这两处必须跟着改，否则派生写回处类型不通。UI 文案随之从「战力 +1234」改为「战力 +12.3%」。
...
寻宝令那一行初稿写的是散文「每 5 层额外 ×2」，可以读成"每层 1 枚、第 5 层起改发 2 枚"（34 层 46 枚、90 层 126 枚），也可以读成"只有 5 的倍数层发 2 枚"（34 层 12 枚、90 层 36 枚）。取后者：`额外` 指在其余三项之上追加，非 5 倍数层不发令牌；这与本节"补供给不足"的定位相称，而前一种读法会让塔单独超过挂机全年产出。
...
底栏七个入口已满（功法/法宝/炼丹/炼器/灵宠/宗门/历练），其中「灵宠」只是跳转到法宝面板的重复入口（`test/feature-rail.test.ts` 专门断言了这一点）。塔占用「灵宠」的槽位：布局坐标不变，七槽等宽不动，灵宠装备槽本身仍在法宝面板内可用。`docs/art-asset-guide.md` 底栏表中 `pet.png` 一行改为 `trial-tower.png`。
...
- `test/loadout-power-model.test.ts`：标尺校准断言——满配 `powerBonusBp` 为 71,774、起步装为 2,809、Lv.11 满配总战力 17,990、Lv.11 起步装总战力 2,817、配装占比在 Lv.11 / 100 / 1000 三点恒定 87.77%、洞府炼器室 Lv.10 贡献 +20%。所有断言按逐件 `floor` 累加取值。
...
- 塔在 Lv.15 解锁时，起步装（六槽普通 +0、四功法普通 1 星，总战力 3,842）可通第 1 层（3,000）与第 2 层（3,540），第 3 层（4,178）不可通——配装检查生效，补品质或强化即可越过，而满配同一层只需 Lv.6。
- 塔的任一层不能重复领奖；界面上不存在塔的扫荡入口。
- 底栏第五个入口为试炼塔而非灵宠；灵宠装备槽在法宝面板内仍可装卸。
