...
|---|---|
| 文档版本 | `0.4.0` |
| 文档状态 | 基线已冻结；第一阶段进行中，核心修炼、挂机资产、角色档案、背包/收获箱、基础道具使用及功法/法宝装备闭环已完成，弱网生命周期、表现与平台验收仍未完成 |
...
| 自动化验证 | 已通过 | 共享领域 29 项、服务端及客户端单元 37 项、PostgreSQL 集成 23 项，共 89 项测试通过；全工程类型检查和服务端构建通过。 |
| Cocos 客户端底座 | 已完成首批 | 启动场景入口、平台适配、API 客户端、Store、固定四 Tab、个人档案入口、角色形象确认、改名输入、锁定页和代码绘制占位主视觉已落地；资产 mutation 携带角色版本并在冲突时同步快照；浏览器已完成登录、定时同步、离线汇总、背包/收获箱及功法/法宝装备、替换、卸下实测，四类资产列表现可完整分页操作。 |
...
| 下一开发项 | 待继续 | 后台生命周期和断网操作队列、升级/突破动效及开发调试面板；微信开发者工具和真机验收仍依赖外部环境。 |
...
| Cocos 客户端底座 | ✅ 已完成 | 启动入口、平台适配、API 客户端、Store、加载/错误页、四 Tab、个人档案面板、首次形象确认、改名输入、锁定页、版本校验与冲突恢复 | 本地权威快照缓存、断网状态、后台生命周期同步和调试面板未实现 |
...
| 微信小游戏平台验收 | 🟡 部分完成 / ⛔ 外部阻塞 | `0.4.0-r11` 源码已生成完整微信小游戏产物，并静态核验角色档案 API、输入处理和 `GameBootstrap` 运行时模块绑定；`0.3.0` Web 产物曾完成浏览器交互实跑 | 尚未在微信开发者工具和真机完成验收；本机 Cocos 全局 `layout.json`、`window.json` 损坏会导致 CLI 在产物完成后仍以非零码退出 |
| Git 版本基线 | ✅ 已完成 | 已创建并推送首个基线提交 `d9e7b83`，后续变更可以审阅、比较和回滚 | 持续保持小批次提交并由 CI 验证 |

当前第一阶段最优先的未完成链路是：`后台/断网恢复 → 升级突破动效与调试面板 → 微信工具和真机验收`。
...
第一阶段规划的 17 条业务接口当前为：**14 条完整实现、2 条部分实现、1 条未实现**。
...
本清单只有已经形成可验收闭环的项目才勾选。未勾选项后标注“部分”时，表示已有底层代码或占位实现，但仍不能按完整需求验收。当前共列出 **49 项，39 项已完成、10 项仍需完成或继续验收**。
...
- [ ] 最新源码可由 Cocos 构建微信小游戏，并在微信开发者工具和真机运行。（部分：`0.4.0-r11` 源码产物已完整生成，并核验角色档案 API、输入处理和 `GameBootstrap` 模块绑定；微信开发者工具/真机未验收）
...
10. `🟡 部分完成`：`0.4.0-r11` 微信小游戏构建与运行时模块绑定核验、既有 Web 浏览器在线/离线实跑和 `0.4.0` 自动化测试已完成；微信开发者工具、真机和完整第一阶段验收未完成。
