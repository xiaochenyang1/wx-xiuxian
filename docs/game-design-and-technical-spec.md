...
|---|---|
| 文档版本 | `0.3.0` |
| 文档状态 | 基线已冻结；第一阶段进行中，核心修炼、挂机资产、背包/收获箱及基础功法/法宝装备闭环已完成，角色编辑、弱网生命周期、表现与平台验收仍未完成 |
...
| 认证与角色初始化 | 已完成首批 | 模拟登录、微信换码适配边界、自动建号、全服唯一原创道号、JWT、刷新令牌轮换、幂等登录和 bootstrap 已实现。微信真实换码等待 AppID/AppSecret。 |
| 自动化验证 | 已通过 | 共享领域 28 项、服务端及客户端单元 31 项、PostgreSQL 集成 17 项，共 76 项测试通过；全工程类型检查和服务端构建通过。 |
| Cocos 客户端底座 | 已完成首批 | 启动场景入口、平台适配、API 客户端、Store、固定四 Tab、锁定页和代码绘制占位主视觉已落地；资产 mutation 携带角色版本并在冲突时同步快照；最新 Web 与微信小游戏产物已生成，浏览器完成登录、定时同步、离线汇总、背包/收获箱及功法/法宝装备、替换、卸下实测。 |
...
| 资产与基础养成 | 已完成第一阶段基础闭环 | 50→200 格扩包、100 格收获箱、转移、分解、满箱品质保护、功法收录/重复本、四类功法装备及六部位法宝装备已接通；每次装备变化由服务端重算战力及经验、灵石、掉落效率。 |
| 下一开发项 | 待继续 | 经验丹使用与角色形象/改名、后台生命周期和断网操作队列、升级/突破动效及开发调试面板；微信开发者工具和真机验收仍依赖外部环境。 |
...
|---|---|---|---|
| pnpm/Cocos/服务端工作区 | ✅ 已完成 | 工作区、统一类型检查、共享包、服务端构建和本地依赖编排可用 | 尚未建立 CI/CD；正式部署按规划延期 |
...
| 开发登录与角色初始化 | ✅ 已完成 | 开发账号登录、自动建号、唯一道号、JWT、刷新轮换、幂等登录和 Bootstrap | 主角形象选择、免费改名和改名卡接口未实现 |
| 微信登录 | 🟡 部分完成 / ⛔ 外部阻塞 | 微信 `code` 换取身份的适配边界和客户端 `wx.login` 路径已实现 | 缺 AppID/AppSecret，未完成真实微信换码、账号实测和异常场景验收 |
| Cocos 客户端底座 | ✅ 已完成 | 启动入口、平台适配、API 客户端、Store、加载/错误页、四 Tab、锁定页、版本校验与冲突恢复 | 本地权威快照缓存、断网状态、后台生命周期同步和调试面板未实现 |
| 在线修炼与自动升级 | ✅ 已完成 | 服务端分段结算、等级变速、合法溢出、灵石微量余数、掉落尝试时钟、五类独立随机掉落和 30 秒客户端同步 | 限时效果及跨配置版本的长区间分段随对应后续系统交付 |
| 境界瓶颈与突破 | ✅ 已完成 | Lv.10 瓶颈、Lv.8 突破丹奖励、缺丹回滚、突破事务、旧境界预结算、Lv.11 解锁和幂等并发保护 | 升级、战力增长和突破专用表现动效未完成 |
| Lv.1000 版本上限 | 🟡 部分完成 | 共享状态机可进入 `version_cap` 并累积修为储备 | 缺服务端集成、客户端专用文案/界面和端到端验收 |
| 基础经济 | 🟡 部分完成 | 灵石产出、余额、累计收入、资产流水和突破丹堆叠已接通 | 仙玉来源、动态奖励箱、主要经济出口和通用道具使用未实现 |
...
| 背包与挂机收获箱 | 🟡 第一阶段核心操作完成 | 50→200 格事务扩容、堆叠道具、100 格收获箱、收取、主动分解、满箱白绿自动分解、蓝色以上保护、幂等 API 和真实操作面板已完成 | 经验丹等通用道具使用、完整筛选/排序、批量管理和邮件兜底系统仍未完成 |
...
| 美术、音频与动效 | 🟡 部分完成 | 当前客户端有代码绘制的原创占位主视觉和基础按钮反馈 | 正式人物/场景/UI 资源、音频、升级与突破特效未制作 |
| 微信小游戏平台验收 | 🟡 部分完成 / ⛔ 外部阻塞 | `0.3.0` 最新源码已重新生成 Web 和微信小游戏构建产物；浏览器 WebGL 登录、在线/离线结算、背包、功法和六法宝槽交互实跑通过 | 未在微信开发者工具和真机完成验收；本机 Cocos 全局 `layout.json`、`window.json` 损坏会导致 CLI 在产物完成后仍以非零码退出 |
| Git 版本基线 | ⬜ 未完成 | 已初始化 Git 元数据 | 仓库仍无首个提交，全部工程文件处于未跟踪状态，尚不能可靠审阅差异或回滚 |

当前第一阶段最优先的未完成链路是：`Git 基线保护 → 经验丹使用与角色编辑 → 后台/断网恢复 → 升级突破动效与调试面板 → 微信工具和真机验收`。
...
- [ ] **部分完成**：基础战力计算及数字增长动画。权威公式和数值展示已完成，增长/升级/突破动效未完成。
- [x] **已完成**：基础功法获取、查看、装备、替换和卸下；服务端同步重算功法战力及修炼效率。
- [x] **已完成**：基础法宝获取、查看、六部位装备、替换和卸下；饰品左右槽及服务端战力/挂机效率重算已验证。
- [ ] **部分完成**：背包、挂机收获箱和基础道具使用。扩容、收取、分解、满箱保护和 UI 已完成；经验丹通用使用接口仍未完成。
- [ ] **部分完成**：最小新手任务链。Lv.8 自动发放突破丹及状态已完成，其他新手引导和任务 UI 未完成。
- [ ] **部分完成**：客户端缓存、服务端同步、幂等结算和异常恢复。会话缓存、定时同步、版本冲突和幂等已完成；快照缓存、断网状态与离线操作队列未完成。
- [x] **已完成**：伴侣、排行、洞府的未开放或未解锁页面。
- [ ] **部分完成**：开发调试面板和核心公式自动化测试。自动化测试已完成首批，调试面板未实现。

按“整条需求全部完成才勾选”的严格口径，第一阶段 12 条高层交付项当前为 **6 条完成、6 条部分完成或未完成**；具体剩余范围以 `0.5` 和第 29 章为准。
...
离线装备操作重连后按操作编号顺序提交；服务端校验基础版本和物品所有权。冲突时以服务端为准，客户端展示简短同步提示并刷新快照。

### 9.5 心跳与缓存

- 在线时每 30 秒发送一次轻量同步心跳，具体间隔可配置。
...
- Cocos 收到切后台事件时立即保存本地缓存，并尽力发送结算请求；请求失败不影响下一次服务端时间结算。
- 本地缓存使用 `wx.setStorageSync`/异步存储封装；浏览器预览使用 `localStorage` 适配器。
- 缓存包含 schema 版本、服务端快照版本和最后成功同步时间。
- 本地数据仅用于快速启动，不是排行榜或资产的权威来源。
...
| POST | `/sync/heartbeat` | 同步在线游标和待提交的允许操作 | 是 | ⬜ 未实现；当前客户端临时每 30 秒调用修炼结算 |
| POST | `/player/avatar` | 首次选择主角形象 | 是 | ⬜ 未实现 |
| POST | `/player/rename` | 免费或道具改名 | 是 | ⬜ 未实现 |
...
| POST | `/inventory/use` | 使用允许的消耗品 | 是 | ⬜ 未实现 |
...
第一阶段规划的 17 条业务接口当前为：**11 条完整实现、2 条部分实现、4 条未实现**。
...
4. 断网允许的装备操作以 UUID 操作号、基础版本和顺序号排队。
5. 重连后逐条验证，失败操作不影响后续可独立操作；最后拉取新快照。
...
- 增加经验、等级和战力测试属性。
- 增加灵石、突破丹、功法和法宝。
- 模拟 1 小时、8 小时和 24 小时离线。
- 跳转到境界瓶颈前一秒或直接修满经验条。
- 切换断网、延迟、超时和版本冲突。
- 固定掉落随机种子以复现测试。
- 清除指定测试账号数据，必须二次确认且只允许开发环境。

调试面板不得通过隐藏按钮保留在生产构建中，必须由构建条件完全排除。
...
本清单只有已经形成可验收闭环的项目才勾选。未勾选项后标注“部分”时，表示已有底层代码或占位实现，但仍不能按完整需求验收。当前共列出 **49 项，36 项已完成、13 项仍需完成或继续验收**。
...
- [ ] 断网时明确标识本地预览状态，重要经济操作不可执行。（未实现）
- [ ] 切后台时保存快照并尽力同步，回到前台后自动恢复。（未实现）
- [ ] 允许的离线装备操作重连后能成功同步或明确回滚。（未实现）
...
- [ ] 升级、战力变化和突破均有可结束、可恢复的占位动效。（未实现专用动效）
- [ ] 最新源码可由 Cocos 构建微信小游戏，并在微信开发者工具和真机运行。（部分：最新源码产物已重新生成，微信开发者工具/真机未验收）
...
- [x] 开发模拟登录路由不会注册到生产环境。
- [ ] 仅开发构建可见的调试面板已经实现并排除于生产构建。（调试面板未实现）
- [ ] 建立自动化 CI，持续执行类型检查、单元测试、数据库集成测试和构建。（未实现）
- [ ] 创建首个 Git 基线提交，使后续变更可审阅、可比较、可回滚。（未完成；当前全部文件未跟踪）
...
4. `✅ 已完成`：实现模拟登录、自动建号、唯一名称和 Bootstrap。
5. `✅ 已完成首批`：在 Cocos 中建立启动场景、平台适配、API 客户端、Store 和固定导航。
...
9. `🟡 部分完成`：锁定页已完成；升级/突破占位动效和开发调试面板未完成。
10. `🟡 部分完成`：最新 Web/微信小游戏构建、浏览器在线/离线实跑和自动化测试已完成；微信开发者工具、真机和完整第一阶段验收未完成。
