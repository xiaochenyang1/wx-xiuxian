...
行囊、法宝、功法三个面板的列表行会在左侧空档里各画一枚图标。程序侧已经接好：把文件放进下表的目录就会生效，不需要再改代码。

**缺图是正常状态。** 没有对应文件的那一行退回纯文字排版，且位置一像素不动——图标画在行**外**的空档里，不占行内空间。所以这套图标可以一张一张地补，补一张生效一张。

### 图标画在哪

面板体 `700` 宽、列表行 `600` 宽，都居中，所以行的两侧各有 50 px 空档。图标画在左侧那条空档里，中心 `x = -326`（以面板中心为原点），按 `contain` 等比缩放：
...
这三组数由 `test/asset-icon-art.test.ts` 钉住：它断言图标既不越出面板体，也不压到行的底板上。收获箱那一列暂时没有图标。
...
程序按「先找逐件、再回落槽位」认领，两种粒度共用一个目录，不需要新目录：

- 法宝行先找 `art/slots/equipment/{EQUIPMENT_CONFIGS 的 id}.png`，没有才用 `{槽位}.png`
- 功法行先找 `art/slots/technique/{TECHNIQUE_CONFIGS 的 id}.png`，没有才用 `{槽位}.png`
- 道具本来就是逐件（`art/items/{ITEM_CONFIGS 的 id}.png`），没有槽位可回落

所以想给天阶那把 `void_immortal_sword` 单独出一张图，直接丢进 `art/slots/equipment/` 就行，其它剑仍然用 `weapon.png`。

### 方案 B：按配置逐件出图（65 张）

20 件装备 + 32 本功法 + 13 个道具，文件名对应 `EQUIPMENT_CONFIGS` / `TECHNIQUE_CONFIGS` / `ITEM_CONFIGS` 里的 `id`。这是上面那条回落规则的极限——把每一格都填满，槽位图就再也用不到了。

### 取舍

- 工作量差 3 倍（22 对 65），而列表里图标只占 `42 x 42` 设计像素，逐件出图的细节在这个尺寸上大半看不出来。
- 方案 B 把美术钉在配置表上：以后加第 5 段位要再补 13 张（5 件装备 + 8 本功法）。方案 A 与段位解耦，加段位一张图都不用补。
- 所以推荐 A，需要时再对个别重点物件补逐件图。

统一 `256 x 256` 透明 PNG，与其它图标一致。列表里按 `contain` 缩到 `42 x 42`（道具堆叠行 `36 x 36`），所以主体要占满画布、别留大边距，细描边缩放后会消失。
