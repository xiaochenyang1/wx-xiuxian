...
import { DROP_CONFIG_VERSION } from "../assets/scripts/services/local-game-snapshot";
...
/**
 * Everything but the daily block. `local-2.16.0` → `local-2.17.0` is the first
 * link in the chain that adds a field rather than only bumping the version, so a
 * save older than that comes back with today's daily progress rebuilt from zero
 * — the one thing a "nothing else moved" comparison cannot include.
 */
function exceptDaily(snapshot: BootstrapSnapshot): Omit<BootstrapSnapshot, "daily"> {
  const { daily: _daily, ...rest } = snapshot;
  return rest;
}
...
describe("local-1.0.0 migration", () => {
  ...
  it("chains through both migrations and backfills every new subsystem", () => {
    ...
    expect(service.snapshot.config.version).toBe("local-2.17.0");
...
describe("local-2.3.0 to local-2.4.0 migration", () => {
  ...
  it("chains a local-1.0.0 save all the way to local-2.11.0", () => {
    const legacy = legacySave();
...
describe("local-2.7.0 migration", () => {
  ...
  it("bumps the version and leaves the rest of the save byte-identical", () => {
    ...
    expect(migrated.config.version).toBe("local-2.17.0");
    ...
    // same bytes out.
    expect(exceptDaily(migrated)).toEqual(exceptDaily(load(head).snapshot));
  });
...
describe("local-2.10.0 migration", () => {
  /** A head save parked at the load instant, so two loads are comparable. */
  function atLoadInstant(): MutableSave {
    const save = authenticSaveWithProgress();
    save.savedAt = LATER.toISOString();
  ...
  function preRealmSplitSave(): MutableSave {
    const save = atLoadInstant();
    save.snapshot.config.version = "local-2.10.0";
    return save;
  }
  ...
  it("bumps the version and leaves the rest of the save byte-identical", () => {
    const head = atLoadInstant();
    const legacy = JSON.parse(JSON.stringify(head)) as MutableSave;
    legacy.snapshot.config.version = "local-2.10.0";
  ...
  it("re-derives a save that still records the old 500-level realm", () => {
    const legacy = atLevel(preRealmSplitSave(), 700);
    legacy.snapshot.progress.realmId = "true_immortal";
    legacy.snapshot.progress.realmName = "真仙期";
    legacy.snapshot.progress.title = "真仙中期";

    const progress = load(legacy).snapshot.progress;

    expect(progress.level).toBe(700);
    expect(progress.realmId).toBe("golden_immortal");
    expect(progress.realmName).toBe("金仙期");
    expect(progress.title).toBe("金仙圆满");
    // Power and the bar are what the tower ladder and the expedition gates are
    // measured against, so they have to survive the split untouched.
    expect(progress.totalPower).toBe("700000000");
    expect(progress.requiredExperience).toBe(requiredExperienceForLevel(700));
  });

  it("credits no breakthrough for the hundreds an old save already passed", () => {
    const legacy = atLevel(preRealmSplitSave(), 700);
    const bag = JSON.parse(
      JSON.stringify(legacy.snapshot.inventory),
    ) as MutableSave;
  ...
  it("does not discard a valid pre-realm-split save", () => {
    expect(rejected(preRealmSplitSave())).toBe(false);
  });
});

describe("local-2.11.0 migration", () => {
  function atLoadInstant(): MutableSave {
    const save = authenticSaveWithProgress();
    save.savedAt = LATER.toISOString();
    save.snapshot.progress.settledAt = LATER.toISOString();
  ...
  function preDaoSave(): MutableSave {
    const save = atLoadInstant();
    save.snapshot.config.version = "local-2.11.0";
    delete save.snapshot.dao;
    return save;
  }
  ...
  it("leaves the rest of the save byte-identical", () => {
    const head = atLoadInstant();
    const legacy = JSON.parse(JSON.stringify(head)) as MutableSave;
    legacy.snapshot.config.version = "local-2.11.0";
    delete legacy.snapshot.dao;

    expect(exceptDaily(load(legacy).snapshot)).toEqual(
      exceptDaily(load(head).snapshot),
    );
  });

  it("credits no attainment and keeps the reserve for the save to spend", () => {
    const legacy = preDaoSave();
    legacy.snapshot.progress.cultivationReserve = "50000000";

    const progress = load(legacy).snapshot.progress;

    // No back-credit: the reserve is the currency, so an old capped save buys
    // its own levels instead of being handed them.
    expect(load(legacy).snapshot.dao.level).toBe(0);
    expect(progress.cultivationReserve).toBe("50000000");
    expect(progress.experienceBonusBp).toBe(0);
  });

  it("does not discard a valid pre-dao save", () => {
    expect(rejected(preDaoSave())).toBe(false);
  });
});

describe("local-2.12.0 migration", () => {
  /**
   * A stored technique row the way the service writes one, so the fixture is a
   * save that actually owns books rather than a pristine one where "nothing
  ...
  /** Parked at the load instant so two loads are comparable byte-for-byte. */
  function atLoadInstant(): MutableSave {
    const save = authenticSaveWithProgress();
    save.savedAt = LATER.toISOString();
    save.snapshot.progress.settledAt = LATER.toISOString();
    save.snapshot.techniques = [
      techniqueRow("quiet_breathing_art", { star: 4, equippedSlot: "mind" }),
      techniqueRow("light_step_art", { duplicateCount: 2 }),
    ];
    return save;
  }

  function preTechniqueBandsSave(): MutableSave {
    const save = atLoadInstant();
    save.snapshot.config.version = "local-2.12.0";
    return save;
  }

  it("bumps the version and leaves every band 1 book exactly as it was", () => {
    const head = atLoadInstant();
    const legacy = JSON.parse(JSON.stringify(head)) as MutableSave;
    legacy.snapshot.config.version = "local-2.12.0";
    ...
    // bytes in, same bytes out — including the bonus snapshots on each row.
    expect(exceptDaily(migrated)).toEqual(exceptDaily(load(head).snapshot));
  });

  it("still resolves every stored book to band 1", () => {
    const service = load(preTechniqueBandsSave());
    expect(service.snapshot.techniques.length).toBe(2);
    for (const item of service.snapshot.techniques) {
      expect(techniqueBandForConfig(item.techniqueConfigId)).toBe(1);
    }
  });

  it("keeps the stars, duplicates and equipped slot the save was holding", () => {
    const service = load(preTechniqueBandsSave());
    const mind = service.snapshot.techniques.find(
      (item) => item.techniqueConfigId === "quiet_breathing_art",
    );

    expect(mind?.star).toBe(4);
    expect(mind?.equippedSlot).toBe("mind");
    expect(
  ...
  it("does not discard a valid pre-technique-band save", () => {
    expect(rejected(preTechniqueBandsSave())).toBe(false);
  });
});

describe("local-2.13.0 migration", () => {
  /** Every building, the sect and the bond parked at what used to be max. */
  function maxedUnderOldCaps(save: MutableSave): MutableSave {
    save.snapshot.cave.buildings = CAVE_BUILDING_CONFIGS.map((config) => ({
      buildingConfigId: config.id,
      level: CAVE_MAX_LEVEL,
    }));
    save.snapshot.sect = {
      sectId: "qingyun",
    ...
    save.snapshot.partner = {
      partnerId: "jun_rulan",
      level: PARTNER_MAX_LEVEL,
      bond: 0,
    };
    save.snapshot.config.version = "local-2.13.0";
    return save;
  }
  ...
  it("bumps the version and leaves the rest of the save byte-identical", () => {
    ...
    const legacy = JSON.parse(JSON.stringify(head)) as MutableSave;
    legacy.snapshot.config.version = "local-2.13.0";
  ...
  it("does not discard a save that was maxed under the old caps", () => {
    expect(rejected(maxedUnderOldCaps(atLevel(authenticSaveWithProgress(), 500)))).toBe(
      false,
    );
  });

  it("reopens the four idle buildings for a Lv.500 save that had them all at 10", () => {
    const service = load(maxedUnderOldCaps(atLevel(authenticSaveWithProgress(), 500)));
    const band = equipmentBandForLevel(service.snapshot.progress.level);

    // Lv.500 is 天阶, so the four idle buildings can now reach 40 while 炼器室 is
    // genuinely finished at 10.
    expect(band).toBe(4);
    for (const building of service.snapshot.cave.buildings) {
      expect(building.level).toBe(CAVE_MAX_LEVEL);
      expect(caveMaxLevelForBand(building.buildingConfigId, band)).toBe(
        building.buildingConfigId === "crafting_room"
          ? CAVE_MAX_LEVEL
          : CAVE_ABSOLUTE_MAX_LEVEL,
      );
    }
    expect(service.snapshot.sect.level).toBe(SECT_MAX_LEVEL);
    expect(service.snapshot.partner.level).toBe(PARTNER_MAX_LEVEL);
  });
});

describe("local-2.14.0 migration", () => {
  it("bumps the version and leaves the rest of the save byte-identical", () => {
    const head = atLevel(authenticSaveWithProgress(), 500);
    const legacy = JSON.parse(JSON.stringify(head)) as MutableSave;
    legacy.snapshot.config.version = "local-2.14.0";
  ...
  it("keeps pills brewed at the old flat price usable and unchanged", () => {
    const legacy = atLevel(authenticSaveWithProgress(), 500);
    legacy.snapshot.config.version = "local-2.14.0";
    legacy.snapshot.inventory.stacks.push({
      itemConfigId: "exp_pill_large",
      displayName: getItemConfig("exp_pill_large").displayName,
      quantity: "7",
    });

    const service = load(legacy);

    // Old stock survives untouched: nothing records the band a pill was brewed
    // at, and using one has always been priced off the level at the moment of
    // use, so a 凡阶-brewed pill is worth 天阶 hours — the §10.2 trade.
    expect(
      service.snapshot.inventory.stacks.find(
...
describe("local-2.16.0 migration", () => {
  it("adds the daily block and leaves the rest of the save byte-identical", () => {
    const head = atLevel(authenticSaveWithProgress(), 500);
    const legacy = JSON.parse(JSON.stringify(head)) as MutableSave;
    legacy.snapshot.config.version = "local-2.16.0";
    // A genuine save of that vintage has no daily block at all — that is what
    // this step exists to add.
    delete legacy.snapshot.daily;
  ...
  it("hands the migrated save a fully built day rather than the -1 sentinel", () => {
    const legacy = atLevel(authenticSaveWithProgress(), 500);
    legacy.snapshot.config.version = "local-2.16.0";
    delete legacy.snapshot.daily;

    const daily = load(legacy).snapshot.daily;

    // The migration writes `dayIndex: -1` with no task rows, and the settlement
    // that immediately follows rolls it into today. The player never sees the
    // sentinel, which is why the migration does not need to know the date.
    expect(daily.dayIndex).toBe(localDayIndex(LATER));
    expect(daily.tasks.map((task) => task.taskConfigId)).toEqual(
      DAILY_TASK_CONFIGS.map((config) => config.id),
    );
    expect(daily.checkedInAt).toBeNull();
    expect(daily.checkInCount).toBe(0);
  });
});
...
describe("cave save validation", () => {
  ...
  it("rejects levels outside 0..maxLevel", () => {
    expect(
      corruptCave((save) => {
        save.snapshot.cave.buildings[0].level = CAVE_ABSOLUTE_MAX_LEVEL + 1;
      }),
    ).toBe(true);
    // The bound is per building, so 炼器室 still refuses Lv.11 while 聚灵阵 at the
    // same level is a legal (band-capped) save.
    expect(
      corruptCave((save) => {
        save.snapshot.cave.buildings[3].level = CAVE_MAX_LEVEL + 1;
      }),
    ).toBe(true);
    expect(
      corruptCave((save) => {
